...
function createCanvasH3Runtime(options = {}) {
  ...
  async function reconcile(ownerId, projectId, jobId) {
    ...
    try {
      ...
      if (result.status === 'failed') return await jobs.updateOwned(ownerId, projectId, jobId, {status:'failed',providerSubmitState:'failed',publicError:'视频生成失败，请检查输入后重试。'});
      ...
      return await jobs.updateOwned(ownerId, projectId, jobId, {status:error?.code === 'CANVAS_H3_BILLING_UNVERIFIED' ? 'review' : 'failed',providerSubmitState:error?.code === 'CANVAS_H3_BILLING_UNVERIFIED' ? 'billing_review' : 'failed',publicError:publicFailure(error)});
    }
  }
  return {enabled,dryRun,submit,reconcile};
}

module.exports = {createCanvasH3Runtime};
