...
const {IMAGE_4K_SIZES, YUNWU_GENERATE_ASPECT_RATIOS, YUNWU_EDIT_ASPECT_RATIOS} = require('./niannian_canvas_aspect_ratios');
...
const GENERATE_OUTPUT_SIZES = Object.fromEntries(YUNWU_GENERATE_ASPECT_RATIOS.map(ratio => [ratio, IMAGE_4K_SIZES[ratio]]));
const EDIT_OUTPUT_SIZES = Object.fromEntries(YUNWU_EDIT_ASPECT_RATIOS.map(ratio => [ratio, IMAGE_4K_SIZES[ratio]]));
...
function createYunwuAgentVaultImage2Adapter(options = {}) {
  ...
  function validate(task, referenceFiles = []) {
    ...
    const expectedSize = isEdit ? EDIT_OUTPUT_SIZES[aspectRatio] : GENERATE_OUTPUT_SIZES[aspectRatio];
...
async function executionExitFailure(result, receiptPath) {
  ...
  return {dryRun, submit, query, constants:{generateEndpoint:'https://yunwu.ai/v1/images/generations', editEndpoint:'https://yunwu.ai/v1/images/edits', model:'gpt-image-2-c', generateOutputSizes:GENERATE_OUTPUT_SIZES, editOutputSizes:EDIT_OUTPUT_SIZES, editOutputSize:EDIT_OUTPUT_SIZE}};
}
