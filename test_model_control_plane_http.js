...
async function run() {
  ...
  assert.equal(JSON.stringify(dolaCatalog).includes('test-dola-key'), false);
  ...
  assert.equal(configuredBody.providerStatus.videoSubmitEnabled, true);
  assert.equal(configuredBody.providerStatus.dolaSubmitEnabled, true);
