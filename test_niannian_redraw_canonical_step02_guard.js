...
async function writeForgedAcceptanceSet(dataRoot, project, variant) {
  ...
  await writeJson(path.join(root, 'artifact_ledger.json'), {schema_version:'artifact_ledger_v1',project_id:project.id,artifacts:[{artifact_id:'step02_acceptance_manifest',status:variant === 'unverified' ? 'pending' : 'verified',sha256:acceptanceSha,downstream_consumable_by:variant === 'unverified' ? [] : ['Step04']}]});
...
async function main() {
  ...
  try {
    ...
    for (const variant of ['unverified','stale','foreign','tampered']) {
    ...
    process.stdout.write(JSON.stringify({ok:true,verified:['acceptance missing returns 409/no-write','artifact Ledger not verified returns 409/no-write','source mismatch and authority revision mismatch return 409/no-write','all Step02-completing and Step04+ controller statuses require exact server reducer acceptance','productionStatus and status aliases guarded','projects/status/events tree byte and mtime unchanged on every rejection','controller may report step02_return_ready only','server reducer remains sole acceptance authority']}) + '\n');
  } finally {
    server.kill();
    await delay(100);
