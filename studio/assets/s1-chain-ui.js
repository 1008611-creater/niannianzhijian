(function () {
  ...
  function mount() {
    ...
    async function saveLayout() {
      var positions = {};
      Object.keys(nodeIds).forEach(function (key) { var pos = positionOf(key); positions[nodeIds[key]] = {x:pos.x,y:pos.y}; });
    ...
    function installDragging() {
      panel.querySelectorAll('.s1-node h3').forEach(function (handle) {
        handle.addEventListener('pointerdown', function (event) {
          var card = event.currentTarget.closest('.s1-node'); var key = card.getAttribute('data-node'); var id = nodeIds[key]; var start = positionOf(key); var baseX = event.clientX; var baseY = event.clientY;
    ...
    async function createImage2() {
      ...
      try {
        ...
        revision = Number(result.body.revision || revision); renderNodes(result.body.document ? result.body.document.nodes : [result.body.node]); image2Card.dataset.status = result.body.node.status; image2Card.querySelector('[data-node-status]').textContent = result.body.node.status; image2Status.textContent = 'Image2 节点已保存。下一步只能由用户明确授权后准备并提交任务。'; image2Dry.disabled = false;
