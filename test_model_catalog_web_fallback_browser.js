...
async function main() {
  ...
  assert.match(generationController, /webCatalogModels as In,webCatalogVendors as Dn/);
  assert.match(generationController, /const s=\(\)=>Dn\(\)\.catch\(/);
  assert.match(generationController, /const i=\(\)=>In\(ct\(e\)\)\.catch\(/);
