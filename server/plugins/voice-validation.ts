...
function validateMimo(input: VoiceRequest, text: string): void {
  if (text.length > 5_000) throw new Error('MiMo text must be at most 5000 characters');
  if (input.outputFormat !== undefined && input.outputFormat !== 'wav' && input.outputFormat !== 'mp3') {
    throw new Error('MiMo outputFormat must be wav or mp3');
  }
  if (input.performancePrompt && input.performancePrompt.length > 500) throw new Error('MiMo style instruction must be 500 characters or fewer');
  reject([input.stability, input.speed, input.similarityBoost, input.style, input.useSpeakerBoost, input.languageCode,
    input.seed, input.optimizeStreamingLatency, input.enableLogging, input.applyTextNormalization,
    input.applyLanguageTextNormalization, input.pronunciationDictionaryLocators, input.previousText, input.nextText,
    input.previousRequestIds, input.nextRequestIds, input.speedRatio, input.emotion, input.emotionScale,
    input.loudnessRatio, input.pitch, input.volume, input.explicitDialect, input.sampleRate, input.bitrate,
    input.audioFormat, input.channel, input.forceCbr, input.stream, input.excludeAggregatedAudio,
    input.languageBoost, input.textNormalization, input.latexRead, input.pronunciations, input.timbreWeights,
    input.voiceModify, input.subtitleEnable, input.subtitleType], 'MiMo only accepts text, voiceId, outputFormat, and performancePrompt');
}

function validateOpenaiTts(input: VoiceRequest, text: string): void {
  if (text.length > 10_000) throw new Error('OpenAI 兼容 TTS 文本最多 10000 个字符');
  range(input.speed, 0.25, 4, 'speed');
  if (!OPENAI_TTS_FORMATS.has(input.outputFormat ?? 'mp3')) throw new Error('OpenAI 兼容 TTS 不支持该 outputFormat');
  reject([input.stability, input.similarityBoost, input.style, input.useSpeakerBoost, input.languageCode,
    input.seed, input.optimizeStreamingLatency, input.enableLogging, input.applyTextNormalization,
    input.applyLanguageTextNormalization, input.pronunciationDictionaryLocators, input.previousText, input.nextText,
    input.previousRequestIds, input.nextRequestIds, input.speedRatio, input.emotion, input.emotionScale,
    input.loudnessRatio, input.pitch, input.volume, input.performancePrompt, input.explicitDialect,
    input.sampleRate, input.bitrate, input.audioFormat, input.channel, input.forceCbr, input.stream,
    input.excludeAggregatedAudio, input.languageBoost, input.textNormalization, input.latexRead,
    input.pronunciations, input.timbreWeights, input.voiceModify, input.subtitleEnable, input.subtitleType],
  'OpenAI 兼容 TTS 只接受 text、voiceId、modelId、outputFormat 和 speed');
}

export function validateVoiceRequest(input: VoiceRequest): ValidVoiceRequest {
  const provider = input.provider;
  if (provider !== 'elevenlabs' && provider !== 'doubao' && provider !== 'minimax' && provider !== 'mimo' && provider !== 'openai-tts'
    && provider !== 'inworld' && provider !== 'fishaudio' && provider !== 'speechify') {
    throw new Error('provider must be elevenlabs, doubao, minimax, mimo, openai-tts, inworld, fishaudio, or speechify');
  }
  const text = String(input.text ?? '').trim();
  const requestedVoiceId = String(input.voiceId ?? '').trim();
  const voiceId = requestedVoiceId || (provider === 'minimax' && !input.timbreWeights?.length ? 'female-yujie' : provider === 'mimo' ? '冰糖' : '');
  ...
  else if (provider === 'mimo') validateMimo(input, text);
  else if (provider === 'openai-tts') validateOpenaiTts(input, text);
  else {
  ...
  return { ...input, provider, text, voiceId, outputFormat: provider === 'mimo' ? input.outputFormat ?? 'wav' : provider === 'openai-tts' ? input.outputFormat ?? 'mp3' : input.outputFormat ?? 'mp3_44100_128',
