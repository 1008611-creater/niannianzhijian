...
/** Xiaomi MiMo V2.5 TTS uses the OpenAI-compatible chat endpoint and returns base64 audio. */
export function mimoVoiceBody(options: VoiceOptions, input: ValidVoiceRequest): Record<string, unknown> {
  // MiMo's official TTS examples require a user style instruction followed by
  // an assistant message containing the text to synthesize. Keep a neutral
  // default so normal editor requests conform even without a custom style.
  const messages: Array<{ role: 'user' | 'assistant'; content: string }> = [
    { role: 'user', content: input.performancePrompt || '请用清晰、自然、适合短视频解说的普通话表达。' },
    { role: 'assistant', content: input.text },
  ];
  return {
    model: input.modelId || options.mimoModel,
    messages,
    // MiMo's official OpenAI-compatible schema calls this field `voice`.
    // `prest_voice` is ignored by the service and can leave a request waiting.
    audio: { format: input.outputFormat, voice: input.voiceId },
  };
}

export async function mimoVoice(options: VoiceOptions, input: ValidVoiceRequest): Promise<Buffer> {
  if (!options.mimoApiKey) throw new Error('MiMo TTS is not configured. Set MIMO_TTS_API_KEY in .env.local or 设置面板.');
  const response = await fetch(`${options.mimoBaseUrl.replace(/\/$/, '')}/chat/completions`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'api-key': options.mimoApiKey },
    // A provider-side stalled connection used to leave the editor's generation
    // card running forever.  Bound it here so the caller gets a normal error
    // and can retry without restarting the editor.
    signal: AbortSignal.timeout(60_000),
    body: JSON.stringify(mimoVoiceBody(options, input)),
  });
  if (!response.ok) throw new Error(await providerError(response));
  ...
  if (!audio.length) throw new Error('MiMo returned empty audio');
  return audio;
}

/**
 * OpenAI-compatible TTS is intentionally a small adapter. It covers self-hosted
 * gateways and aggregators that implement POST /audio/speech without leaking
 * provider-specific fields into the shared request contract.
 */
export function openaiTtsVoiceBody(model: string, input: ValidVoiceRequest): Record<string, unknown> {
  return {
    model,
    input: input.text,
    voice: input.voiceId,
...
export async function openaiTtsVoice(options: VoiceOptions, input: ValidVoiceRequest): Promise<Buffer> {
  if (!options.openaiTtsApiKey) throw new Error('OpenAI 兼容 TTS 未配置。请在设置面板填写 OPENAI_TTS_API_KEY。');
  const response = await fetch(`${options.openaiTtsBaseUrl.replace(/\/$/, '')}/audio/speech`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${options.openaiTtsApiKey}` },
    body: JSON.stringify(openaiTtsVoiceBody(input.modelId || options.openaiTtsModel, input)),
  });
  if (!response.ok) throw new Error(await providerError(response));
  const audio = Buffer.from(await response.arrayBuffer());
  if (!audio.length) throw new Error('OpenAI 兼容 TTS 返回空音频');
  return audio;
}
