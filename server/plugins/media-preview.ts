...
async function runDerivative<T>(
  ...
  // A preview proxy is reusable cache material, not a one-shot response. React
  // can briefly replace the preview component while the editor is rendering;
  // do not kill an FFmpeg proxy job merely because that first request closed.
  // The completed proxy will be served to the next request. Other derivatives
  // retain their cancellation behavior.
  const release = () => {
    if (!continueOnDisconnect) lease.release();
  };
  ...
  return {
    ...
    configureServer(server) {
      ...
      const proxyDeps = {
        ...
        runDerivative: <T>(req: IncomingMessage, res: ServerResponse, key: string, work: DerivativeWork<T>, protectedPath?: string) => (
          runDerivative(req, res, key, work, protectedPath, true)
        ),
