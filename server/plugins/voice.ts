...
import { doubaoVoice, elevenLabsVoice, fishAudioVoice, inworldVoice, minimaxVoice, mimoVoice, openaiTtsVoice, speechifyVoice } from './voice-providers.ts';
...
function audioDescriptor(provider: VoiceProvider, outputFormat: string, audioFormat: string, sampleRate: number) {
  ...
  if (provider === 'mimo') return { codec: outputFormat, sampleRate: 24_000 };
  if (provider === 'openai-tts') return { codec: outputFormat, sampleRate: 24_000 };
      ...
      server.middlewares.use('/generate/voice', async (req, res) => {
        ...
        try {
          ...
          if (input.provider === 'minimax') {
           ...
           else if (input.provider === 'mimo') bytes = await mimoVoice(options, input);
           else if (input.provider === 'openai-tts') bytes = await openaiTtsVoice(options, input);
