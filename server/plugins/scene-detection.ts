...
export async function detectScenesInFile(
  ...
  const filter = [
    ...
    // Keep the metadata branch non-empty even when no frame crosses the
    // threshold. FFmpeg 8.x on Windows can fail a zero-frame null output
    // before the progress branch has a chance to finish; normalization below
    // applies the threshold to the printed scene scores.
    `[scan]select='gte(scene,0)',metadata=print:file=-[hits]`,
