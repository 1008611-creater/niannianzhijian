...
export function planExport(body: ExportRequest | null): ExportPlan {
  ...
  let smartCutJobId: `SCJ-${string}` | undefined;
  if (body?.project !== undefined) {
    ...
    const smartCut = (migratedProject as ProjectDoc & {
      niannianSmartCut?: { jobId?: unknown };
    }).niannianSmartCut;
    if (typeof smartCut?.jobId === 'string' && /^SCJ-[a-f0-9]{24}$/.test(smartCut.jobId)) {
      smartCutJobId = smartCut.jobId as `SCJ-${string}`;
    }
  ...
  if (body?.smartCutJobId !== undefined) {
    if (typeof body.smartCutJobId !== 'string' || !/^SCJ-[a-f0-9]{24}$/.test(body.smartCutJobId)) {
      throw new ExportRequestError('smartCutJobId must be a valid smart-cut job id');
    }
    smartCutJobId = body.smartCutJobId as `SCJ-${string}`;
  }
