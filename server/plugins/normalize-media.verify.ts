...
const missingTargetRealpath = async (path: string): Promise<string> => {
  ...
  if (portablePath.startsWith('/alias/')) {
    throw Object.assign(new Error(`missing: ${path}`), { code: 'ENOENT' });
  }
  assert.equal(portablePath, '/alias');
...
async function findFreePort(): Promise<number> {
  ...
  if (!address || typeof address === 'string') throw new Error('could not allocate a local verification port');
  return address.port;
}
...
try {
  ...
  const verificationPort = await findFreePort();
  server = await createServer({
    ...
    server: { host: '127.0.0.1', port: verificationPort, strictPort: true },
  });
  await server.listen();
