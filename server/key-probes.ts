...
export const PROBES: Record<string, ProbeDef> = {
  ...
  'voice/mimo': {
    needs: [['MIMO_TTS_API_KEY']],
    run: (get) => fetch(`${base(get, 'MIMO_TTS_BASE_URL', 'https://api.xiaomimimo.com/v1')}/models`, {
      signal: t(), headers: { 'api-key': get('MIMO_TTS_API_KEY') },
    }),
    models: parseModelCatalog,
  },
  'voice/openai-tts': {
    needs: [['OPENAI_TTS_API_KEY']],
    run: (get) => fetch(`${base(get, 'OPENAI_TTS_BASE_URL', 'https://api.openai.com/v1')}/models`, {
      signal: t(), headers: bearer(get('OPENAI_TTS_API_KEY')),
    }),
    models: parseModelCatalog,
  },
  'analysis/openai-vision': {
    needs: [['OPENAI_VISION_API_KEY']],
    run: (get) => fetch(`${base(get, 'OPENAI_VISION_BASE_URL', 'https://api.openai.com/v1')}/models`, {
      signal: t(), headers: bearer(get('OPENAI_VISION_API_KEY')),
    }),
    models: parseModelCatalog,
  },
  'transcription/openai-asr': {
    needs: [['OPENAI_ASR_API_KEY']],
    run: (get) => fetch(`${base(get, 'OPENAI_ASR_BASE_URL', 'https://api.openai.com/v1')}/models`, {
      signal: t(), headers: bearer(get('OPENAI_ASR_API_KEY')),
    }),
    models: parseModelCatalog,
  },
  // MiMo ASR deliberately reuses the MiMo TTS account key and base URL.
