...
export function hwDecodeArgs(encoder?: H264Encoder): string[] {
  ...
  switch (process.platform) {
    ...
    // Windows builds often advertise d3d11va even when the current process has
    // no usable Direct3D device (CI, WSL-hosted Node, or headless sessions).
    // Let FFmpeg probe and fall back to software instead of failing a filter
    // graph before it can emit any frames.
    case 'win32': return ['-hwaccel', 'auto'];
...
export async function resolveHwDecodeArgs(ffmpeg: string, encoder?: H264Encoder): Promise<string[]> {
  ...
  if (candidate[1] === 'vaapi' && !existsSync(resolveVaapiDevice())) return [];
