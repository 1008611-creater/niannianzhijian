...
import { seedKeystore, getKey, runtimeSettingsPath } from './server/keystore.ts';
...
export default defineConfig(({ mode }) => {
  ...
  const mimoKey = env.MIMO_TTS_API_KEY || '';
  ...
  return {
    ...
    define: {
      ...
      __CONFIGURED_CAPS__: JSON.stringify({
        ...
        voice: Boolean((doubaoAppId && doubaoAccessKey) || elevenKey || mimoKey || minimaxKey),
    ...
    server: {
      port: 5199,
      strictPort: true,
      // This instance is a long-running public service, not a local development
      // session. Disabling HMR also disables React's Fast Refresh transform,
