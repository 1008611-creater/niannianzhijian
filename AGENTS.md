...
- 已验证根因：服务器模型配置仍存在，但网页桥接或旧缓存读取到空目录时，画布会误报“未接入”。服务器模型目录与供应商状态是唯一事实源；网页端必须保留同源目录回退、健康状态回退和版本化资源地址。
- 每次涉及模型目录、桥接脚本或静态入口的发布，必须运行 `test_model_catalog_web_fallback_browser.js`，并由 `scripts/run_quality_gate.js` 纳入质量门；测试必须覆盖模型、价格、供应商映射、健康状态和入口缓存版本。
- 空目录只能在服务器确实返回空目录时显示；桥接失败、缓存过期、接口未登录或请求异常必须显示可恢复的实际状态，不得伪装成“模型未接入”。发布后须从全新页面回读桥接对象、入口版本和模型目录，旧标签页只能作为线索。

### S-011 生成规格持久化契约

- 已验证根因：服务器模型目录已经声明比例、清晰度和输出尺寸，但网页目录适配器未将这些字段编译为生成节点可渲染的 `imageOptions` / `videoOptions`，导致用户看不到比例与大小，任务也可能回落到默认值。
- 所有生图和生视频模型必须由服务器模型目录声明可用比例与大小；网页节点必须显式渲染对应控件，选择值必须写回节点元数据并随任务请求提交，不能只改显示文案。
...
- 体验关：默认由代理为 PR 当前精确提交提供独立 HTTPS 预览并完成真实浏览器回归，公开健康回读必须证明预览 SHA 与 PR HEAD 完全一致；若产品负责人已明确只看正式线上效果，独立预览仅供代理内部验证，不作为用户验收入口，发布后必须从正式主站回读真实结果。
...
- 当产品负责人明确表示“只看上线的网站”或“不需要独立预览”时，不再要求其打开临时预览或 GitHub PR 页面；代理仍须先完成技术检查和内部回归，得到明确的“验收通过，发布”授权后，将已验证候选部署到正式主站，再只提供正式线上网址、候选提交 SHA、改动摘要、风险/回滚点和电脑/手机各一条核心路径。
- 验收清单必须写成用户可执行动作和预期结果，例如“打开网址 → 登录 → 进入项目 → 上传/选择素材 → 执行节点 → 刷新 → 确认状态和结果仍在”；未提供线上地址和清单时不得要求用户验收。
- 线上候选只在得到“验收通过，发布”后合并并切换生产；新提交、候选地址变化或部署回滚都会使上一次验收失效。

### 正式线上验收偏好

- 产品负责人只检查正式主站的真实效果；独立预览可以继续用于代理内部调试、自动化测试和回滚前验证，但不得作为用户必须打开的验收页面，也不能替代正式主站回读。
- 每次正式上线后，必须主动报告正式网址、实际部署提交、用户可见改动、风险与回滚点，以及最短电脑/手机验收路径；本轮没有生产变更时，必须明确说明未部署。
