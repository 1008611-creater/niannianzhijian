...
const after = 'const renameRequestRef=y.useRef(0),po=y.useCallback(()=>{const h=P.current;Yr(h);const w=ve.current;ve.current=null,w?.(),P.current=null,Wt(null),d(null),i("library"),n(It(),{replace:!1}),gv(),a()},[n,a]),Z=y.useCallback(h=>{if(!s)return;const w=h.trim()||e("appBar.untitledProject");if(w===s.name)return;const N={...s,name:w},requestId=++renameRequestRef.current;Oe().then(async({service:K})=>{if(K.projects&&typeof K.projects.updateMetadata==="function")return K.projects.updateMetadata(s.id,{name:w});const{readCurrentWorkbenchProjectPayload:L}=await fe(async()=>{const{readCurrentWorkbenchProjectPayload:oe}=await Promise.resolve().then(()=>Wa);return{readCurrentWorkbenchProjectPayload:oe}},void 0,import.meta.url);return K.persistProject(N,L())}).then(K=>{if(requestId!==renameRequestRef.current)return;d(K&&K.name?{...s,...K,name:K.name}:N)}).catch(K=>{if(requestId!==renameRequestRef.current)return;console.error("project rename save error",K),Ee(e("studio.renameFailed"),"error")})},[s,Oe,e])';
