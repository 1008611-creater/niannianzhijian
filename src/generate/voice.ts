...
export interface SubmitVoiceArgs {
  provider: 'elevenlabs' | 'doubao' | 'minimax' | 'mimo' | 'openai-tts' | 'inworld' | 'fishaudio' | 'speechify';
  text: string;
  voiceId: string;
  modelId?: string;
  stability?: number;
...
export async function submitVoice(args: SubmitVoiceArgs, state: TimelineState): Promise<MediaAsset> {
  ...
  const props = {
    generation,
    ...(result.subtitlePath
      ? { minimaxSubtitlePath: result.subtitlePath, minimaxSubtitleType: args.subtitleType ?? 'sentence' }
      : {}),
  };
  return {
    id: newId(),
