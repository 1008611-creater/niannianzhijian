...
export interface TranscribeOptions {
  /** Provider is explicit per job. AssemblyAI remains the compatibility default. */
  provider?: 'assemblyai' | 'openai-asr' | 'mimo-qwen-asr';
  /**
...
/** MiMo supplies transcript text; local Qwen supplies the real timestamps. */
export async function transcribeMimoQwenPath(path: string, opts: TranscribeOptions = {}): Promise<TranscriptResult> {
  const response = await serviceFetch('/api/mimo-qwen-asr/transcribe', {
    method: 'POST', headers: { 'content-type': 'application/json' },
    body: JSON.stringify({ src: path, language: opts.languageCode ?? 'zh' }),
  });
  const body = await response.json().catch(() => ({})) as OpenAiAsrResponse;
  if (!response.ok || body.ok !== true) throw new Error(typeof body.error === 'string' ? body.error : `MiMo + Qwen ASR failed (${response.status})`);
  const words = (body.words ?? []).flatMap((word) => {
    const text = typeof word.text === 'string' ? word.text.trim() : '';
    const start = Number(word.start);
    const end = Number(word.end);
    return text && Number.isFinite(start) && Number.isFinite(end) && end >= start ? [{ text, start, end, speaker: null }] : [];
  });
  if (!words.length) throw new Error('MiMo + Qwen ASR returned no aligned timestamps');
  return { text: typeof body.text === 'string' ? body.text : words.map((word) => word.text).join(''), words, utterances: [] };
}
...
/** OpenAI-compatible ASR runs through the local server, which owns the API key. */
export async function transcribeOpenAiCompatiblePath(path: string, opts: TranscribeOptions = {}): Promise<TranscriptResult> {
  const response = await serviceFetch('/api/openai-asr/transcribe', {
    method: 'POST', headers: { 'content-type': 'application/json' },
    body: JSON.stringify({ src: path, languageCode: opts.languageCode ?? 'zh' }),
  });
  const body = await response.json().catch(() => ({})) as OpenAiAsrResponse;
  if (!response.ok || body.ok !== true) throw new Error(typeof body.error === 'string' ? body.error : `OpenAI compatible ASR failed (${response.status})`);
  const words = (body.words ?? []).flatMap((word) => {
    const text = typeof word.text === 'string' ? word.text.trim() : '';
    const start = Number(word.start);
    const end = Number(word.end);
  ...
  if (!words.length) throw new Error('OpenAI compatible ASR returned no word timestamps');
  return { text: typeof body.text === 'string' ? body.text : words.map((word) => word.text).join(' '), words, utterances: [] };
}
...
export async function transcribePath(
  ...
  if (opts.provider === 'openai-asr') return transcribeOpenAiCompatiblePath(path, opts);
  // MiMo + Qwen is the product default: it returns model-derived timestamps
  // without depending on AssemblyAI billing. AssemblyAI remains explicit-only.
  if (opts.provider !== 'assemblyai') return transcribeMimoQwenPath(path, opts);
