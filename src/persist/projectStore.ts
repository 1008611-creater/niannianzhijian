...
export async function loadProject(id: string, options?: ProjectMigrationOptions): Promise<ProjectDoc | null> {
  return loadProjectDocument(id, () => idbGet<unknown>(projectKey(id)), options);
}

/** Re-read the authoritative shared copy, bypassing this tab's in-memory cache. */
export async function refreshProject(id: string, options?: ProjectMigrationOptions): Promise<ProjectDoc | null> {
  return loadProjectDocument(id, () => idbRefresh<unknown>(projectKey(id)), options);
}
