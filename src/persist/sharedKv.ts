...
/**
 * Read one value from the shared store even when this browser tab already has a
 * cached copy.  Project editors use this when regaining focus so a second tab
 * cannot keep editing an out-of-date timeline after another tab applied work.
 */
export async function kvRefresh<T>(key: string): Promise<T | undefined> {
  await ready();
  if (remoteCache) {
    try {
  ...
  return remoteCache ? remoteCache[key] as T | undefined : localGet<T>(key);
}
