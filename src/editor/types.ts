...
/** Versioned, source-bound analysis produced by OCR/vision/audio workers. */
export interface AssetIntelligenceEntity {
...
export interface AssetIntelligenceScene {
  id: string;
  startMs: number;
  endMs: number;
  label?: string;
...
export interface MediaAsset {
  ...
  /** Optional OCR/vision/audio metadata. Every derived result must name its source revision. */
...
/** Narrow, non-secret context linking an editor project to a NOMI smart-cut job. */
export interface NiannianSmartCutContext {
  jobId: `SCJ-${string}`;
  sourceAssetId?: `CAS-${string}`;
  preset: 'talking_head' | 'explainer' | 'short_video';
  aspectRatio: string;
  captionStyle: string;
...
export interface ProjectDoc {
  ...
  /** Optional bridge context; contains no URLs, credentials, or user session data. */
  niannianSmartCut?: NiannianSmartCutContext;
}
