...
export const TRANSCRIPT_TOOL_SCHEMAS: AgentToolSchema[] = [
  ...
  },
  {
    ...
    description: 'Transcribe every audio/video clip on a track and attach word-level timestamps. The mimo-qwen-asr route uses MiMo for transcript text and the local Qwen3 ForcedAligner for real timestamps, avoiding AssemblyAI credits. AssemblyAI remains the compatibility default; openai-asr requires verbose_json word timestamps. Required before find_transcript / clean_script / delete_text / captions when clips have no transcript yet.',
    input_schema: { type: 'object', properties: {
      track: { type: 'string', description: 'Track alias or stable id whose audio to transcribe (default A1).' },
      provider: { type: 'string', enum: ['assemblyai', 'openai-asr', 'mimo-qwen-asr'], description: 'ASR provider. mimo-qwen-asr = MiMo text + local Qwen3 ForcedAligner timestamps; openai-asr = configured compatible provider; omit for AssemblyAI compatibility.' },
    } },
  },
  {
    name: 'search_media',
    description: 'Search project media through one typed surface. Returns visual ChineseCLIP scene hits, spoken transcript hits, and source-bound OCR/tag/entity metadata hits with normalized scores, source time ranges, asset ids, and source revisions. Results are grouped by modality because scores are not directly comparable; stale derived hits are excluded.',
    input_schema: {
      type: 'object',
      properties: {
        ...
        modalities: {
          type: 'array',
          items: { type: 'string', enum: ['visual', 'spoken', 'metadata'] },
  ...
  },
  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        provider: { type: 'string', enum: ['assemblyai', 'openai-asr'], description: 'retry_transcription only: choose the configured ASR provider. Omit for AssemblyAI compatibility default.' },
