...
const mimoVoice = (args: GenerateArgs): SubmitVoiceArgs => ({
  ...voiceBase(args, 'mimo'), outputFormat: str(args.outputFormat), performancePrompt: str(args.performancePrompt),
});
const openaiTtsVoice = (args: GenerateArgs): SubmitVoiceArgs => ({
  ...voiceBase(args, 'openai-tts'), modelId: str(args.modelId), outputFormat: str(args.outputFormat), speed: num(args.speed),
});

const VOICE_STRATEGIES = {
  elevenlabs: elevenVoice, doubao: doubaoVoice, minimax: minimaxVoice, mimo: mimoVoice, 'openai-tts': openaiTtsVoice,
...
export function buildSubmitVoiceArgs(args: GenerateArgs): SubmitVoiceArgs {
  const provider = args.provider === 'doubao' || args.provider === 'minimax' || args.provider === 'mimo' || args.provider === 'openai-tts' || args.provider === 'inworld'
