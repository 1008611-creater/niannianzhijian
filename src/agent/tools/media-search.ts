...
export async function execSearchMedia(args: SearchMediaArgs, ctx: AgentContext): Promise<unknown> {
  ...
  const requested = Array.isArray(args.modalities)
    ...
    : new Set(['visual', 'spoken', 'metadata']);
  const invalid = [...requested].filter((value) => value !== 'visual' && value !== 'spoken' && value !== 'metadata');
  ...
  const result = buildMediaSearchResult(query, visual.slice(0, limit), spoken.slice(0, limit), metadata.slice(0, limit), assets);
  return {
    ...result,
