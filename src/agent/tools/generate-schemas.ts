...
export const GENERATE_TOOL_SCHEMAS: AgentToolSchema[] = [
  ...
  {
    name: 'submit_voice',
    description: 'Generate one TTS audio asset with ElevenLabs, Doubao, MiMo, OpenAI-compatible TTS, MiniMax, Inworld, Fish Audio, or Speechify. Creates an asset only; it does not place or replace timeline items. Confirm a provider and voice; MiniMax timbreWeights may mix voices with an empty voiceId.',
    input_schema: {
      type: 'object',
      properties: {
        provider: { type: 'string', enum: ['elevenlabs', 'doubao', 'mimo', 'openai-tts', 'minimax', 'inworld', 'fishaudio', 'speechify'], description: 'elevenlabs for multilingual/non-Chinese; doubao for Chinese-optimized speech; mimo for Xiaomi MiMo V2.5 TTS; openai-tts for any OpenAI-compatible /audio/speech endpoint; minimax for MiniMax Chinese TTS; inworld, fishaudio, and speechify are general multilingual TTS (text + voiceId + optional modelId only).' }, // provider enum
        ...
        modelId: { type: 'string', description: 'ElevenLabs, MiMo, OpenAI-compatible TTS, Inworld, Fish Audio, or Speechify model override. Defaults to the configured current model.' },
        stability: { type: 'number', minimum: 0, maximum: 1, description: 'ElevenLabs only. Defaults to 0.5.' },
        speed: { type: 'number', minimum: 0.25, maximum: 4, description: 'ElevenLabs (0.7–1.2), MiniMax (0.5–2), or OpenAI-compatible TTS (0.25–4). Server enforces the selected provider.' },
        ...
        outputFormat: { type: 'string', description: 'Provider output format: ElevenLabs official enum, MiMo wav/mp3, or OpenAI-compatible mp3/opus/aac/flac/wav/pcm.' },
        ...
        performancePrompt: { type: 'string', maxLength: 500, description: 'Doubao or MiMo performance/style direction.' },
