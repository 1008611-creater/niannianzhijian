...
8. After each TTS segment finishes, read the real audio duration before placing
   ...
   on the timeline yet. Keep the generated source unlinked and make no partial
   voiceover mutation to the timeline for that render batch. Fix the mismatch
...
- Do not treat a generated TTS asset as ready for placement just because it
  ...
  first.
- When any generated narration exceeds its matching visual window, publish no
  voiceover items from that render batch. Return the affected visual range and
  real duration, then tighten, split, or deliberately extend the visual before
  retrying.
