...
import { ROUGH_CUT_TOOL_NAMES, ROUGH_CUT_TOOL_SCHEMAS } from './tools/schemas/rough-cut-tools';
...
import { FORCED_ALIGNER_TOOL_NAMES, FORCED_ALIGNER_TOOL_SCHEMAS } from './tools/forced-aligner-tools';
...
export const TOOL_SCHEMAS: AgentToolSchema[] = [
  ...
  // Known text + matching generated narration audio -> actual Qwen forced-alignment timestamps.
  ...FORCED_ALIGNER_TOOL_SCHEMAS,
  ...
  ...SCRIPT_TOOL_SCHEMAS,
  // A selected beat list becomes a separate editable timeline with source-bound clips.
  ...ROUGH_CUT_TOOL_SCHEMAS,
...
const EXECUTOR_GROUPS: ReadonlyArray<readonly [ReadonlySet<string>, ToolExecutorLoader]> = [
  ...
  [FORCED_ALIGNER_TOOL_NAMES, async () => (await import('./tools/forced-aligner-tools')).execForcedAlignerTool],
  ...
  [ROUGH_CUT_TOOL_NAMES, async () => (await import('./tools/rough-cut-tools')).execRoughCutTool],
