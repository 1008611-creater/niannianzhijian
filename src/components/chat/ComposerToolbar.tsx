...
function BarBtn({ icon, title, onClick, active, disabled, className, expanded, hasPopup, label }: {
  icon: IconName;
  title: string;
  ...
  return (
    <button type="button" title={title} aria-label={title} onClick={onClick} disabled={disabled}
      ...
      <Icon name={icon} size={16} />{label && <span className="cc-composer-tool-label">{label}</span>}
    </button>
  );
}
    ...
    <div className="cc-chat-composer-bar">
      <div className="cc-chat-composer-bar-tools">
        ...
        <span className="cc-composer-secondary">
          ...
          <BarBtn icon="plus" title={t('引用媒体池素材')} label={t('引用素材')} active={pop === 'assets'} className="cc-composer-reference-btn" onClick={(event) => onTogglePop('assets', event.currentTarget)} />
