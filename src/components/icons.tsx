...
export function BrandMark({ size = 16 }: { size?: number }) {
  return (
    <img src="/niannian-logo.svg" alt="" aria-hidden width={size} height={size} style={{ display: 'block' }} />
  );
}

/** 念念智剪 product word mark. */
export function OpenChatCutWordmark({ width = 126 }: { width?: number }) {
  return (
    <span
      aria-label="念念智剪"
      style={{ display: 'block', flexShrink: 0, width, fontFamily: 'system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif', fontSize: Math.round(width / 4.1), fontWeight: 760, letterSpacing: '-0.045em', lineHeight: 1, whiteSpace: 'nowrap' }}
    >
      念念智剪
    </span>
  );
