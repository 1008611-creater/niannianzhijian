...
export function TopBar({ projectId, projectName, canUndo, canRedo, exporting, exportJobCount = 0, onHome, onRename, onResumeGeneration }: TopBarProps) {
  ...
  useEffect(() => {
    let alive = true;
    fetch('/api/niannian-auth/credits', { cache: 'no-store' })
      .then((response) => response.ok ? response.json() as Promise<{balance?: unknown}> : null)
      .then((body) => { if (alive && body && Number.isFinite(Number(body.balance))) setCreditsBalance(Math.max(0, Math.trunc(Number(body.balance)))); })
      .catch(() => undefined);
    return () => { alive = false; };
  }, []);
  ...
  return (
    <header className={`cc-topbar cc-window-titlebar${isMacDesktop ? ' cc-window-titlebar--mac' : ''}`} style={{ gridColumn: '1 / -1', gridRow: 1, position: 'relative', height: '100%', display: 'flex', alignItems: 'center', padding: '0 6px', borderBottom: `0.5px solid ${theme.border}`, background: theme.panel, gap: 4 }}>
      ...
      <ExportHistory />
      <LocaleToggle />
      {creditsBalance !== null && <span aria-label={t('积分余额')} style={{ color: theme.textDim, fontSize: 11, whiteSpace: 'nowrap', padding: '0 4px' }}>{t('积分')} {creditsBalance}</span>}
