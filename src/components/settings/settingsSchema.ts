    ...
    groups: [
      ...
      { key: 'voice', title: '配音 / TTS', hint: 'submit_voice · 文字转配音，任一厂商即可。',
        ...
        vendors: [
          ...
          { key: 'voice/mimo', vendor: 'mimo', title: '小米 MiMo TTS',
            note: '支持预置音色、音色设计和音色复刻模型；当前接入预置音色配音。', fields: [
              secret('MIMO_TTS_API_KEY', 'API Key'),
              text('MIMO_TTS_BASE_URL', 'Base URL', '默认 https://api.xiaomimimo.com/v1'),
              modelSelect('MIMO_TTS_MODEL', '配音模型', 'mimo-v2.5-tts', ['mimo-v2.5-tts']),
            ] },
          { key: 'voice/openai-tts', vendor: 'openai', title: 'OpenAI 兼容 TTS',
            note: '适用于自建网关、聚合平台和实现 /audio/speech 的本地服务。', fields: [
              secret('OPENAI_TTS_API_KEY', 'API Key'),
              text('OPENAI_TTS_BASE_URL', 'Base URL', '默认 https://api.openai.com/v1'),
              modelText('OPENAI_TTS_MODEL', '配音模型', 'tts-1'),
            ] },
    ...
    groups: [
      ...
      { key: 'transcription', title: '转写 / 口播剪辑', hint: 'transcribe_track · 词级字幕、清口水、删词。',
        route: routeSelect('PREFERRED_TRANSCRIPTION_VENDOR', [
          ...
          { value: 'openai-asr', label: 'OpenAI 兼容 ASR' },
        ]),
        vendors: [
          { key: 'transcription/assemblyai', vendor: 'assemblyai', title: 'AssemblyAI',
            fields: [secret('ASSEMBLYAI_API_KEY', 'API Key')] },
          { key: 'transcription/openai-asr', vendor: 'openai', title: 'OpenAI 兼容 ASR',
            note: '适用于实现 /audio/transcriptions 的官方接口、自建网关和本地服务。所选模型必须支持 verbose_json 的词级时间戳；否则不会用于字幕。',
            fields: [
              secret('OPENAI_ASR_API_KEY', 'API Key'),
              text('OPENAI_ASR_BASE_URL', 'Base URL', '默认 https://api.openai.com/v1'),
              modelText('OPENAI_ASR_MODEL', '转写模型', 'whisper-1'),
            ] },
          { key: 'transcription/mimo-asr', vendor: 'mimo', title: '小米 MiMo ASR（文本）',
            note: '复用「配音 / TTS」中的 MiMo API Key 与 Base URL；支持 MP3/WAV 及本地视频自动提取音频。官方响应不含词级时间戳，因此只用于素材文本理解与检索，不能用于字幕、删词或时间线重剪。',
            fields: [
              secret('MIMO_TTS_API_KEY', '复用的 MiMo API Key'),
              modelText('MIMO_ASR_MODEL', '转写模型', 'mimo-v2.5'),
            ] },
        ] },
      { key: 'asset-analysis', title: '素材视觉分析', hint: 'analyze_asset · 商品、人物、场景标签与检索。',
        vendors: [
          { key: 'analysis/openai-vision', vendor: 'openai', title: 'OpenAI 兼容视觉模型',
            note: '识别时仅发送选定的本地图片或视频抽帧；支持官方接口、自建网关与兼容中转。密钥只保存在本机服务端。',
            fields: [
              secret('OPENAI_VISION_API_KEY', 'API Key'),
              text('OPENAI_VISION_BASE_URL', 'Base URL', '默认 https://api.openai.com/v1'),
              modelText('OPENAI_VISION_MODEL', '视觉模型', 'gpt-4.1-mini'),
            ] },
        ] },
    ],
  },
