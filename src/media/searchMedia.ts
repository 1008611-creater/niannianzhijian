...
export interface MediaSearchResult {
  ...
  metadata: MetadataMediaSearchHit[];
  hits: MediaSearchHit[];
}
...
export function metadataMediaSearchHits(query: string, assets: readonly MediaAsset[], limit = 24): MetadataMediaSearchHit[] {
  return metadataDocumentsForQuery(query, assets, limit).map((document) => ({
    modality: 'metadata', assetId: document.assetId, sourceRevision: document.sourceRevision,
...
export function buildMediaSearchResult(
  ...
  metadata: readonly MetadataMediaSearchHit[] = [],
  assets: readonly MediaAsset[],
): MediaSearchResult {
  const fresh = filterStaleMediaSearchHits([...visual, ...spoken, ...metadata], assets);
  ...
  const spokenHits = fresh
    ...
    .map((hit) => ({ ...hit, score: clampScore(hit.score) }))
    .toSorted((left, right) => right.score - left.score || left.sourceStartMs - right.sourceStartMs);
  const metadataHits = fresh
    .filter((hit): hit is MetadataMediaSearchHit => hit.modality === 'metadata')
    .map((hit) => ({ ...hit, score: clampScore(hit.score) }))
    .toSorted((left, right) => right.score - left.score || left.sourceStartMs - right.sourceStartMs);
  return {
    query,
    ...
    hits: [...visualHits, ...spokenHits, ...metadataHits],
  };
}
