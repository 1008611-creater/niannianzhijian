...
type HeicToArgs = { blob: Blob; type: 'image/jpeg'; quality: number };
type HeicToModule = { heicTo?: (args: HeicToArgs) => Promise<Blob | Blob[]> };
...
async function convertHeic(record: MobileUploadRecord): Promise<File> {
  ...
  const heicTo = (await import('heic-to') as unknown as HeicToModule).heicTo;
  if (!heicTo) throw new Error(t('HEIC 转换失败'));
