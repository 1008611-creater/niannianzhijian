...
async function run() {
  ...
  const sensitiveProviderInput = await fetchJson('/api/admin/model-config/provider', {method:'PUT',headers:{...headers('admin-token'),'content-type':'application/json'},body:JSON.stringify({id:'yunwu-agent-vault',label:'云雾',kind:'image',enabled:true,baseUrl:'https://must-not-be-persisted.example',secretRef:'browser-secret-must-not-be-persisted'})});
  assert.equal(sensitiveProviderInput.response.status, 200);
  ...
  const plan = await fetchJson('/api/admin/commerce/plan', {method:'PUT',headers:{...headers('admin-token'),'content-type':'application/json'},body:JSON.stringify({id:'creator-monthly',label:'个人创作者月度套餐',audience:'creator',monthlyPriceCny:39,monthlyCredits:120,modelIds:['yunwu-gpt-image-2-c'],published:true})});
  assert.equal(plan.response.status, 200);
  const grant = await fetchJson('/api/admin/credits/adjust', {method:'POST',headers:{...headers('admin-token'),'content-type':'application/json'},body:JSON.stringify({tenantId:'TEN-A',amount:20,reason:'test'})});
  assert.equal(grant.response.status, 200);
  assert.equal(grant.body.account.userId, 'SYSTEM-ADMIN');
  const assignment = await fetchJson('/api/admin/commerce/tenant-plan', {method:'PUT',headers:{...headers('admin-token'),'content-type':'application/json'},body:JSON.stringify({tenantId:'TEN-A',planId:'creator-monthly',status:'active'})});
  assert.equal(assignment.response.status, 200);
  const updatedSummary = await fetchJson('/api/admin/commerce/summary', {headers:headers('admin-token')});
  assert.equal(updatedSummary.body.plans.some(item => item.id === 'creator-monthly' && item.monthlyPriceCny === 39), true);
  assert.equal(updatedSummary.body.tenantPlans.some(item => item.tenantId === 'TEN-A' && item.planId === 'creator-monthly'), true);
  assert.equal(JSON.stringify(updatedSummary.body).includes('must-not-be-persisted.example'), false);
  assert.equal(JSON.stringify(updatedSummary.body).includes('browser-secret-must-not-be-persisted'), false);
