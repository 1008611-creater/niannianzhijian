...
**Primary action:** 进入三个正式入口之一。

## Structure

- Hallmark structural reference: `Workbench`.
- The first viewport contains exactly three parallel entry units:
  1. 无限画布
  2. 一键转绘
  3. 一键短剧
- Do not add a fourth tool, a task dashboard, a fake progress bar, or an explanatory marketing section to the first viewport.
...
- 一键短剧 -> project-bound script input route; not an unowned global modal.
...
- Three entries share a visual grammar but are not generic nested cards.
...
- Exactly three entry units are visible in the first viewport.
