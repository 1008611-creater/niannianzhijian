...
function configure(values) {
  return new Promise((resolve, reject) => {
    const child = spawn('ssh', ['haika-niannian', 'install -d -m 700 /etc/niannian-ai; umask 077; cat > /etc/niannian-ai/step01-hq.env; chmod 600 /etc/niannian-ai/step01-hq.env; systemctl restart niannian-ai.service; systemctl is-active niannian-ai.service'], {stdio:['pipe','ignore','pipe'],windowsHide:true});
    let stderr = '';
    ...
    child.stdin.end([
      ...
      'NIANNIAN_STEP01_GPT_MODEL=gpt-5.6-sol',
      ''
    ].join('\n'));
  });
}
const server = http.createServer(async (request, response) => {
  ...
  request.once('end', async () => {
    ...
    if (value.csrf !== csrf || !valid(value.mimo) || !valid(value.paddle) || !valid(value.gpt) || !valid(value.gpt_base) || !/^https:\/\/.+/.test(value.gpt_base)) return send(response, 400, page('<p class="error">请完整填写四项，并使用 HTTPS API 地址。</p>'));
    try { await configure(value); used = true; send(response, 200, '<!doctype html><meta charset="utf-8"><title>已配置</title><p>Haika 配置已写入并且服务已重启为 active。此本地页面将在 3 秒后关闭。</p>'); setTimeout(() => server.close(), 3000); }
